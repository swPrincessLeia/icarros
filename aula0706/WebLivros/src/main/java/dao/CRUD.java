...
public class CRUD extends ConexaoBD {
	...
	// Método para inserir um registro, todos os campos são not null
	// ________________________________________________________________________________________
	public void inserirLivros(Livro livro) {

		try {
			this.preparedStatement = this.connection.prepareStatement(
					"insert into livros(nome_livro, autor_livro, editora_livro, data_lancamento)"+
					" value (?, ?, ?, ?)");
	...
	public void editarLivro(Livro livro) {
		...
		try {
			this.preparedStatement = this.connection.prepareStatement(
					"update livros set nome_livro=?,"+
					" autor_livro=?," +
					" editora_livro=?," +
					"data_lancamento=?" +
					" where id_livros=" + livro.getId());
	...
	// ________________________________________________________________________________________
	// Método que lista registros filtrados
	// ________________________________________________________________________________________

	public ArrayList<Livro> pesquisarLivros(String substring) {
		...
		try {
			...
			String query = "select * from icarros.livros " +
					       "where nome_livro like '%" + substring + "%'" +
					       "or autor_livro like '%" + substring + "%'" +
					       "or editora_livro like '%" + substring + "%'" +
					       "or data_lancamento like '%" + substring + "%'";
